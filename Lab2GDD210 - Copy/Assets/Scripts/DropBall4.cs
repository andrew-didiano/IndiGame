...
public class DropBall4 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (catchCount == 10)
        {
            SceneManager.LoadScene("Win4End"); //if the score is 10, load the final win screen.
        }
        else if (catchCount < 0)
        {
            SceneManager.LoadScene("Lose4"); //if the score is under 0, load the final lose screen.
        }
...
//Used some code from clunk47 on Unity forums to figure out how to set a random x position to the ball whenever the player scores or goes past -5.5 on the screen. 